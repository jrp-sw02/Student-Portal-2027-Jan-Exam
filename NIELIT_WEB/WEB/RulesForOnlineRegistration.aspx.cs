...
public partial class RulesForOnlineRegistration : BasePage
{
   ...
   //         }
    ...
    protected void RenderPage(Int32 courseID)
    {
        try
        {
            using (EConnectContext context = new EConnectContext())
            {
                ...
                {
                    Label1.Text = "Online Registration Application Form For:-" + currentCourse.Name;
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            using (EConnectContext context = new EConnectContext())
            {
                currentCourse = context.Courses.Find(Convert.ToInt32(Request.QueryString["id"]));
                coursecategoryID = currentCourse.CourseCategoryID;
            };
            if (currentCourse.enmCourseType == enmCourseType.CertificationCourse)
            { Response.Redirect("~/CAND/NielitRegistration.aspx?ID=" + Request.QueryString["id"], false); }
            else
            { Response.Redirect("~/CAND/CertificateRegistration.aspx?id=" + Request.QueryString["id"] + "&CoursecategoryID=" + coursecategoryID.ToString() + "&candtype=" + Request.QueryString["candtype"], false); }
        }
        catch (Exception ex)
        { ShowAlert(ex.Message); }
    }
