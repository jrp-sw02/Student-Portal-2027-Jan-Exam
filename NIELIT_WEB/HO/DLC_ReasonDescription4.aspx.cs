...
public partial class Admin_DLC_ReasonDescription4 : BasePage
{
        ...
        try
        {
            using (var context = new EConnectContext())
            {
                ...
                var examName = (from a in context.Exams
                                where (a.ExamMonth == DateTime.Now.Month && a.ExamYear == DateTime.Now.Year && a.CourseID.ToString() == ddlCourseName.SelectedValue)
        ...
        try
        {
            ...
            using (var context = new EConnectContext())
            {
                if (courseName == 175)
                {
                    ...
                    var examIdSelection1 = (from e in context.Exams
                                            where e.ID == examId && e.CourseID == courseName
                                            select new { examId = e.ID }).FirstOrDefault();

                    var examIdTable1 = (from c in context.CertificateExamApplications
                                        where c.Number == applNo && c.ResultGradeID == null
                                        select new { examId = c.ExamID }).FirstOrDefault();

                    if (examIdSelection1.examId == examIdTable1.examId)
                    {
                        validRecord = 1;
                    }
                    else
            ...
            using (var context = new EConnectContext())
            {
                if (CompareExamId(applicationNumber, examMonth, examYear, courseName) == 1)
                {
                    ...
                    try
                    {
                        ...
                        " ,s.[Result_Updated_On],s.[Result_Updated_By],s.[Exempted_Application_ID],s.[Is_Exempted],s.[App_Source],s.[Is_Synced],s.[Synced_On],s.[Exam_Month],s.[Exam_Year],s.[Examn_Cycle_ID],s.[Previous_Exam_Name] " +
                        " ,s.[Aadhar_Number],s.[Aadhar_Verfied],s.[Department],s.[Employee_Code],s.[Designation],s.[Posting_City],s.[Date_of_Joining],s.[Date_of_Retirement],s.[Is_Downloaded],s.[Downloaded_On],s.[UID_Type],s.[UID_Number] " +
                        " ,s.[Email_sent_flag],s.[SMS_sent_flag],s.[Is_Disability],s.[Disability_Type_ID],s.[Disability_Percentage],s.[Is_EWS],s.[onlineRefID],s.[isUmang],@statusId,@currentDate,@userNo " +
                        "  FROM [dbo].[Certificate_Exam_Application] s where s.Number = @applicationNumber)", param1);
                        context.SaveChanges();
                        //}
                        //else
                        //{
                        ...
                        if (readCount > 1)
                        {
                            ...
                            if (CompareExamIdB(applicationNumber, examMonth, examYear, courseName) == 1)
                            {
                                ...
                                else
                                {
                                    ...
                                    //December_2024
                                    SqlParameter[] param2 = { new SqlParameter("@applicationStatus", applicationStatus),
                                                                new SqlParameter("@currentDate", DateTime.Now),
                                                                    new SqlParameter("@userNo", userNo),
                                                                        new SqlParameter("@applicationNumber", applicationNumber)
                                                                            };
                                   ...
                                   " ,s.[Result_Updated_On],s.[Result_Updated_By],s.[Exempted_Application_ID],s.[Is_Exempted],s.[App_Source],s.[Is_Synced],s.[Synced_On],s.[Exam_Month],s.[Exam_Year],s.[Examn_Cycle_ID],s.[Previous_Exam_Name] " +
                                   " ,s.[Aadhar_Number],s.[Aadhar_Verfied],s.[Department],s.[Employee_Code],s.[Designation],s.[Posting_City],s.[Date_of_Joining],s.[Date_of_Retirement],s.[Is_Downloaded],s.[Downloaded_On],s.[UID_Type],s.[UID_Number] " +
                                   " ,s.[Email_sent_flag],s.[SMS_sent_flag],s.[Is_Disability],s.[Disability_Type_ID],s.[Disability_Percentage],s.[Is_EWS],s.[onlineRefID],s.[isUmang],@applicationStatus,@currentDate,@userNo " +
                                   "  FROM [dbo].[Certificate_Exam_Application] s where s.Number = @applicationNumber)", param2);
                                    context.SaveChanges();
                                    //}
                                    //else
                                    //{
    ...
    protected void ddlCourseName_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (ddlCourseName.SelectedValue == "175")
        {
            toggleExamName(true);
            BindExamName();
        }
            ...
            toggleExamName(false);
    }
    void toggleExamName(bool status)
    {
        trExamMonth.Visible = !status;
        trExamYear.Visible = !status;
        trExamName.Visible = status;
    }
}