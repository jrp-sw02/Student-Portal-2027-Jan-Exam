...
public partial class HO_NielitCentreBatchwiseFinancialRepFilter : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            BreadCrumb1.AddNewBreadCrumbItem(new BreadCrumbItem("Nielit Centre Batchwise Financial report", "HO/Rpt/NielitCentreBatchwiseFinancialRepFilter.aspx", ""));
                ...
                using (EConnectContext context = new EConnectContext())
                {
                    ...
                    using (NIELITMISContext context1 = new NIELITMISContext())
                    {
                        ...
                        if (UserTypeId == 10)
                        {
                            ...
                            ddlCentreName.SelectedValue = NielitCentreId.ToString();
                        }
  ...
  protected void BindCentres()
    ...
    public DataTable FillCourseNIELITMISCourseNielitCourseRecord()
    {
        ...
        using (SqlConnection con = new SqlConnection(constr))
        {
            try
            {
                using (SqlCommand cmd = new SqlCommand("[GetFillCourseBatchFinancial]", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                 ...
                 if (coursecatID != 0)
                 {
                     ...
                     using (NIELITMISContext context = new NIELITMISContext())
                     {
                         ...
                         var CourseList = from p in context.NielitCentreCourses 
                                          ...
                                          orderby (p.Name)
                                          select new { ValueField = d.ID, TextField = p.Name + '('+ d.courseDurationDays.ToString()+ "Days-"+ d.courseDurationHrs.ToString()+"Hrs)"   };
    ...
    protected void ddlcourseName_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            Int32 nielitcentreID = Convert.ToInt32(ddlSubcentreName.SelectedValue);
            if (nielitcentreID == 0)
            {
                 ...
                 if (courseID != 0) 
                 {
                     using (NIELITMISContext context = new NIELITMISContext())
                     {
                         ...
                         var CourseList = from b in context.NielitCentreBatchs 
                                          ...
                                          orderby (b.Name)
                                          select new { ValueField = b.ID, TextField = b.BatchCode } ; 
                 ...
                 if (courseID != 0)
                 {
                     using (NIELITMISContext context = new NIELITMISContext())
                     {
                         ...
                         var CourseList = from b in context.NielitCentreBatchs
                                          ...
                                          where d.ID == courseID && b.centreID == nielitcentreID || b.subCentreID == nielitcentreID
                                          orderby (b.Name)
     ...
     protected void ddlSubcentreName_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            ddlbatchname.Items.Add(new ListItem("--Select One--", "0"));
            Int32 nielitcentreID = Convert.ToInt32(ddlSubcentreName.SelectedValue);
            Int32 courseID = Convert.ToInt32(ddlcourseName.SelectedValue);
            Int32 coursecatID = Convert.ToInt32(ddlcoursecategory.SelectedValue);
            Int32 CourseType = coursecatID.ToString().Length;
            ...
            if (CourseType <= 2 )
            {
                if (courseID != 0 && nielitcentreID !=0)
                {
                    using (NIELITMISContext context = new NIELITMISContext())
                    {
                        ListItem lst = new ListItem("--Select One--", "0");
                        var CourseList = from b in context.NielitCentreBatchs
                                         where b.CourseDurationID == courseID && b.centreID == nielitcentreID || b.CourseDurationID == courseID && b.subCentreID == nielitcentreID
                                         orderby (b.Name)
                                         select new { ValueField = b.ID, TextField = b.Name };
                        EConnect.Utils.Common.ControlUtility.BindListObject(ddlbatchname, CourseList.Distinct(), lst);
                    };
            ...
            else
            {
                if (courseID != 0 && nielitcentreID != 0)
                {
                    using (NIELITMISContext context = new NIELITMISContext())
                    {
                        ...
                        var CourseList = from b in context.NielitCentreBatchs
                                         join d in context.NielitCourseDurations on b.CourseDurationID equals d.ID
                                         where d.ID == courseID && b.centreID == nielitcentreID || d.ID == courseID && b.subCentreID == nielitcentreID
                                         orderby (b.Name)
                                         select new { ValueField = b.ID, TextField = b.Name };
                        EConnect.Utils.Common.ControlUtility.BindListObject(ddlbatchname, CourseList.Distinct(), lst);
                    };
                            ...
                            { 
                                ...
                                { 
                                    ...
                                    var Afflcentre = (from p in context.AffInstitutes
                                                      where p.linkedToCentre == NelitCentreLinkId && p.instituteID== loginUser.UserRefNumber 
                                                      select p).FirstOrDefault() ; 
                                    ddlSubcentreName.SelectedValue = Afflcentre.instituteID.ToString() ; 
                                    ddlCentreName.SelectedValue = NelitCentreLinkId.ToString() ; 
                                } 
    ...
    protected void ddlCentreName_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            using (NIELITMISContext context = new NIELITMISContext())
            {
                ...
                NIELITCentreId.Value = ddlcentreId .ToString ();
            }
        }
        catch (Exception ex)
        {
