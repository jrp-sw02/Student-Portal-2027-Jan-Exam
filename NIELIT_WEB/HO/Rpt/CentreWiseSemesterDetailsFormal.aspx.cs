...
public partial class HO_Rpt_CentreWiseSemesterDetailsFormal : System.Web.UI.Page
{
    Table tbl = new Table();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!UserManager.HasRight(currentRoleId, enmRight.View, "Common/CentreWiseSemesterDetailsFilterFormal.aspx"))
            {
    ...
    protected void ShowTableHeader()
    {
        try
        {
            ...
            //tcCol13.Text = "Mob No. (Course Coordinator)";
            tcCol13.Text = "Mobile";
    ...
    protected void ShowData()
    {
        try
        {
            ...
            int courseId = 0, batchId = 0;
            Int64 centreId = 0;

            string reportId = "";
            DateTime batchFromDate = DateTime.MinValue, batchToDate = DateTime.MinValue;

            centreId = Convert.ToInt32(Request.QueryString["centreId"]);
            reportId = Convert.ToString(Request.QueryString["reportId"]);
            ...
            if (reportId == "C")
                ...
                {
                 batchFromDate = Convert.ToDateTime(Request.QueryString["batchFromDate"]);
                 batchToDate = Convert.ToDateTime(Request.QueryString["batchToDate"]);
                }

            using (ds = GetSemesterData(centreId, reportId, courseId, batchId, batchFromDate, batchToDate))
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    ibExport.Visible = true;
                        ...
                        tdRow30.Text = ds.Tables[0].Rows[i]["semesterNo"].ToString();
                        ...
                        tdRow13.Text = ds.Tables[0].Rows[i]["semesterSession"].ToString();
                        tdRow13.HorizontalAlign = HorizontalAlign.Center;
                        tdRow13.Wrap = false;
                        tr.Cells.Add(tdRow13);

                        TableCell tdRow14 = new TableCell();
                        tdRow14.Width = Unit.Percentage(3);
                        tdRow14.Text = ds.Tables[0].Rows[i]["semesterDurationTheoryHours"].ToString();
                        tdRow14.HorizontalAlign = HorizontalAlign.Center;
                        tdRow14.Wrap = false;
                        tr.Cells.Add(tdRow14);

                        TableCell tdRow6 = new TableCell();
                        tdRow6.Width = Unit.Percentage(3);
                        tdRow6.Text = ds.Tables[0].Rows[i]["semesterDurationPracticalHours"].ToString();
                        ...
                        tdRow9.Width = Unit.Percentage(9);
                        ...
                        string EmailF = "";
                        //EmailF = ds.Tables[0].Rows[i]["facultyEmail"].ToString();
                        if (ds.Tables[0].Rows[i]["facultyEmail"] != "")
                            {
                            EmailF = ds.Tables[0].Rows[i]["facultyEmail"].ToString();
                            tdRow10.Text = EmailConvertIntoTextForm(EmailF);
                            }
                        else
                            {
                            tdRow10.Text = "NA";
                        ...
                        if (ds.Tables[0].Rows[i]["mobile"].ToString() != "0")
                            {
                            //EmailF = ds.Tables[0].Rows[i]["facultyEmail"].ToString();
    ...
    public DataSet GetSemesterData(Int64 centreId, string reportId, Int64 courseId, Int64 batchId, DateTime batchFromDate, DateTime batchToDate)
    {
        string constr = ConfigurationManager.ConnectionStrings["NIELITMISContext"].ConnectionString;
        DataTable myDt = new DataTable();
        using (SqlConnection con = new SqlConnection(constr))
        {
        using (SqlCommand cmd = new SqlCommand("GetSemesterhWiseDataReportFormal", con))
            {
                ...
                // only for 0 values
                    batchFromDate = DateTime.MinValue;
                    batchToDate = DateTime.MinValue;
                ...
                else if (reportId == "D")
                {
                    ...
                    // only for 0 values
                    cmd.Parameters.Add(new SqlParameter("@courseId", SqlDbType.BigInt));
                    cmd.Parameters["@courseId"].Value = 0;
                    cmd.Parameters.Add(new SqlParameter("@batchId", SqlDbType.BigInt));
    ...
    protected void ibExport_Click(object sender, ImageClickEventArgs e)
        ...
        try
            ...
            divReportData.Controls.Add(tbl);
            Response.Clear();
            Response.AddHeader("content-disposition", "attachment;filename=CentreWiseSemesterDetailsFormal.xls");
            Response.Charset = "";
    ...
    protected void imgPDF_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=CentreWiseSemesterDetailsFormal.pdf");
