...
public partial class ReportPage : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!UserManager.HasRight(currentRoleId, enmRight.View, "Common/ConsolidatedPaymentApplicationFilter.aspx"))
            {
    ...
    protected void ShowTableHeader()
    {
        try
        {
            ...
            tcCol13.Width = Unit.Percentage(19);
            tcCol13.HorizontalAlign = HorizontalAlign.Center;
            tcCol13.Text = "Course Name";
            ...
            if (TypeId == Convert.ToInt32(enmApplicationType.CertificateExamApplication) || TypeId == Convert.ToInt32(enmApplicationType.CourseExamApplication))
            {
                ...
                tcCol14.Text = "First Centre Choice-City (State)";
                ...
                tcCol15.Text = "Second Centre Choice-City (State)";
            ...
            tcCol17.Width = Unit.Percentage(4);
            ...
            tcCol12.Width = Unit.Percentage(4);
            tcCol12.Text = "Payment Gateway Used";
    ...
    protected void  ShowDataReport()
    {
        ...
        {            
            ...
            Int32 CourseCatId = Convert.ToInt32(Request.QueryString["CourseCatId"]);
            int InstituteID = 0;
            ...
            if (ExamName != null)
            {
                CourseCategory crc = context.CourseCategories.Where(s => s.ID == CourseCatId).FirstOrDefault();              
                strHead += "<b>Course Category Name :</b> " + crc.Name;
                strHead += " , <b> Exam Name : </b>" + ExamName;
            }
            else
            {
            ...
            if ( CourseCatId != 0 )
            {
                ...
                {                    
                    ...
                    var application = (from a in context.CertificateExamApplications.AsNoTracking()
                                       join w in context.Courses on a.CourseID equals w.ID
                                       join ot in context.OnlineTransaction on a.DemandNoteID equals ot.DemandNoteID
                                       join pg in context.Paymentgateways on ot.PGCode equals pg.ID.ToString()

                                       where examid.Contains(a.ExamID) && a.CourseCategoryID == CourseCatId && a.FinalSubmitted == true
                                       select new
                                       {
                                           ID = a.ID,
                                           ...
                                           PaymentGatewayname = pg.Description
                                       }).ToList();                            
                        ...
                        //application = application.OrderBy(s => s.CourseID).ToList();
                    if (application.Count() > 0)
                    {
                        application = application.OrderBy(s => s.CourseID).ThenBy(s => s.Number).ToList();
                        ...
                        for (int i = 0; i < application.Count(); i++)
                        {
                            ...
                            tdRow25.Width = Unit.Percentage(10);
                            tdRow25.Text = application[i].CourseNameP;
                            ...
                            if (loginUserType == UserType.RegionalCenter || loginUserType==UserType.projectNIELITCentre || loginUserType == UserType.HeadOffice || loginUserType == UserType.Admin || loginUserType == UserType.ExternalAdmin)
                            {
                                TableCell tdRow7 = new TableCell();
                            ...
                            if (!String.IsNullOrEmpty(application[i].PaymentGatewayname))
                                tdRow30.Text = application[i].PaymentGatewayname;
                            else
                                tdRow30.Text = "NA";
                            tr.Cells.Add(tdRow30);
                ...
                {                   
                    ...
                    var application = (from a in context.CourseRegistrationApplications.AsNoTracking()
                                       ...
                                       join ot in context.OnlineTransaction on a.DemandNoteID equals ot.DemandNoteID
                                       join pg in context.Paymentgateways on ot.PGCode equals pg.ID.ToString()
                                       where examid.Contains(a.ApplicableExamID.ToString()) && a.CourseCategoryID == CourseCatId && a.FinalSubmitted == true
                                       select new
                                       {
                                           ID = a.ID,
                                           ApplicationDate = a.ApplicationDate,
                    ...
                    if (RegionalcentreID != 0)
                    {
                        RegionalCenter regcenterr = context.RegionalCenters.Where(s => s.ID == RegionalcentreID).FirstOrDefault();
                        strHead += "</br> <b>Regional Centre Name :</b>" + GetInitCap(regcenterr.Name);
                    }
                    if (applicantTypeID == 0)
                        strHead += "</br> <b>Applicant Type :</b>Both";
                    else
                        strHead += "</br> <b>Applicant Type :</br> " + EConnect.Utils.Common.EnumUtility.GetDescription((EConnect.NIELIT.enmApplicantType)(applicantTypeID)).ToString();
                    if (applicantTypeID == 1)
                    {
                        application = application.Where(s => s.ApplicantTypeID == Convert.ToInt32(enmApplicantType.Direct)).ToList();
                    }
                ...
                {  
                    ...
                    if (application.Count() > 0)
                    {
                        ...
                        for (int i = 0; i < app.Count(); i++)
                        {
                            ...
                            TableCell tdRow25 = new TableCell();
                            tdRow25.Width = Unit.Percentage(10);
                            tdRow25.Text = app[i].CourseNameP;
                            tdRow25.HorizontalAlign = HorizontalAlign.Center;
                            tr.Cells.Add(tdRow25);
                ...
                if (TypeId == Convert.ToInt32(enmApplicationType.CertificateExamApplication))
                {
                    ...
                    if (loginUserType == UserType.HeadOffice)
                    {
                        ...
                        else if (CandidateState != 0)
                        {
                            ...
                            // Modified 2 Jan 2019 - No change as all states may be required for report whether new or old.
                            Location ct = context.Locations.Where(s => s.ID == CandidateState && s.LocationTypeID == 2).FirstOrDefault();
                            //  Location ct = context.Locations.Where(s => s.ID == CandidateState && s.LocationTypeID == 2 && s.effectiveToDate ==null).FirstOrDefault();
    ...
    protected void ibExport_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            ...
            Response.ContentType = "application/vnd.xls";
            Response.AddHeader("Content-Disposition", "attachment;filename=ConsolidatedPaymentApplicationReport.xls");
