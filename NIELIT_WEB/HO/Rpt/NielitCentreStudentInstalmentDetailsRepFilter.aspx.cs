...
public partial class HO_NielitCentreStudentInstalmentDetailsRepFilter : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try 
        {
            BreadCrumb1.AddNewBreadCrumbItem(new BreadCrumbItem("Nielit Centre Student Installments Details ", "HO/Rpt/NielitCentreStudentBatchFilter.aspx", ""));
                  ...
                  using (NIELITMISContext context1 = new NIELITMISContext())
                  {
                      ...
                      else if (UserTypeId == 4)
                      {
                          var intituteslinkedToCentre = from s in context1.AffInstitutes
                                                        ...
                                                        select new { ID = s.instituteID, linkedToCentre = s.linkedToCentre };
                          ...
                          var intituteslinkedToCentres = context1.AffInstitutes.Find(loginUser.UserRefNumber);
                 ...
                 if (coursecatID != 0)
                 {
                     using (NIELITMISContext context = new NIELITMISContext())
                     {
                         ...
                         var CourseList = from p in context.NielitCentreCourses 
                                          ...
                                          where p.CourseCategoryID == coursecatID &&  b.centreID == nielitcentreID || b.subCentreID == nielitcentreID
                                          && d.isVerified==true
                                          orderby (p.Name)
                                          select new { ValueField = p.ID, TextField = b.Name + " ( " + d.courseDurationDays  + " Days )" };
