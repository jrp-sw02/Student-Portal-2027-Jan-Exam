...
public partial class HO_NielitCentreStudentBatchFilter : BasePage
{
                  ...
                  using (NIELITMISContext context1 = new NIELITMISContext())
                  {
                      ...
                      else if (UserTypeId == 4)
                      {
                         ...
                         // NonAfflAfflInstID = Convert.ToInt32(intituteslinkedToCentre.FirstOrDefault().ID);
                          NonAfflAfflInstID = Convert.ToInt32(intituteslinkedToCentre.FirstOrDefault().instid);
    ...
    protected void ddlcourseName_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            Int32 coursecatID = Convert.ToInt32(ddlcoursecategory.SelectedValue);
             ...
             Int32 nielitSubcentreID = Convert.ToInt32(HNonAfflAfflInst.Value);
             if (nielitSubcentreID != 99)
             {
                 nielitcentreID = 0;
             }
                 ...
                 if (courseID != 0)
                 {
                     using (NIELITMISContext context = new NIELITMISContext())
                     {
                         ...
                         var CourseList = from b in context.NielitCentreBatchs
                                          where b.CourseDurationID == courseID && b.centreID == nielitcentreID || b.subCentreID == nielitSubcentreID
                                          orderby (b.Name)
                 ...
                 if (courseID != 0)
                 {
                     using (NIELITMISContext context = new NIELITMISContext())
                     {
                         ...
                         var CourseList = from b in context.NielitCentreBatchs
                                          ...
                                          where d.courseID == courseID && b.centreID == nielitcentreID || b.subCentreID == nielitSubcentreID
                                          orderby (b.Name)
                                          select new { ValueField = b.ID, TextField = b.Name };
                         EConnect.Utils.Common.ControlUtility.BindListObject(ddlbatchname, CourseList.Distinct(), lst);
                     };
     ...
     protected void ddlSubcentreName_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            HNonAfflAfflInst.Value = ddlSubcentreName.SelectedValue;
