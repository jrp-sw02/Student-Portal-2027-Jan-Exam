    ...
    protected void btnView_Click(object sender, EventArgs e)
    {
        ...
        else if (PaymentMode == "ONLINE")
        {
            ...
            //if (CourseCode == "CCC" || CourseCode == "BCC" || CourseCode == "CCCP" || CourseCode == "ECC")
            {
                if (ddlDateRange.SelectedItem.Text == "From Start Date")
                {
                    mySql.Append("select e.Name, sum(c.Fee_Amt) as Total_Amount, cast(Settled_On as date) as Online_Settle_Date  " +
                                   "from Online_Transaction  a, Demand_Note b,Certificate_Exam_Application c, Exam_Center  d, Location e " +
                                   "where a.Demand_Note_ID =b.ID and b.ServiceID = @ServiceCodeExam and a.Demand_Note_ID =c.Demand_Note_ID and c.Exam_Center1_ID  =d.ID and d.State_ID =e.ID	 " +
                                   "and a.Settled_On >= CAST(@fromDate as DATE) group by e.Name ,cast(Settled_On as date) order by Online_Settle_Date desc ");
                    //mySql.Append("select e.Name, sum(c.Fee_Amt) as Total_Amount, cast(Settled_On as date) as Online_Settle_Date  " +
                    //                "from Online_Transaction  a, Demand_Note b,Certificate_Exam_Application c, Exam_Center  d, Location e " +
                    //                "where a.Demand_Note_ID =b.ID and b.ServiceID ='" + ServiceCodeExam + "' and a.Demand_Note_ID =c.Demand_Note_ID and c.Exam_Center1_ID  =d.ID and d.State_ID =e.ID	 " +
            ...
            else if (CourseCode == "ACC")
            {
                if (ddlDateRange.SelectedItem.Text == "From Start Date")
                {
                    mySql.Append("select e.Name , sum(c.Fee_Amt) as Total_Amount, cast(Settled_On as date) as Online_Settle_Date " +
                                   ...
                                   "and c.Course_Category_Id = @CourseCatId and c.Candidate_ID is not null  and c.Demand_Note_ID is not null  " +
                                   "and a.Settled_On >= CAST(@fromDate as DATE) group by e.Name , cast(Settled_On as date) order by Online_Settle_Date, e.Name ,Total_Amount ");
                    //mySql.Append("select e.Name , sum(c.Fee_Amt) as Total_Amount, cast(Settled_On as date) as Online_Settle_Date " +
                    //               "from Online_Transaction  a, Demand_Note b,Course_Registration_Application c, Institute  d, Location e " +
                    //               "where a.Demand_Note_ID =b.ID and b.ServiceID ='" + ServiceCodeRegn + "' and a.Demand_Note_ID =c.Demand_Note_ID and d.ID =c.Institute_ID  and d.State_ID =e.ID  " +
                    //               "and c.Course_Category_Id = '" + CourseCatId + "' and c.Candidate_ID is not null  and c.Demand_Note_ID is not null  " +
                    //               "and a.Settled_On >= CAST('" + FromDate.ToString("dd-MMM-yyyy") + "' as DATE) group by e.Name , cast(Settled_On as date) order by Online_Settle_Date, e.Name ,Total_Amount ");
                }
                else
                {
                    mySql.Append("select e.Name , sum(c.Fee_Amt) as Total_Amount, cast(Settled_On as date) as Online_Settle_Date " +
                                 ...
                                 "where a.Demand_Note_ID =b.ID and b.ServiceID = @ServiceCodeExam and a.Demand_Note_ID =c.Demand_Note_ID and d.ID =c.Institute_ID  and d.State_ID =e.ID  " +
                                 "and c.Course_Category_Id = @CourseCatId and c.Candidate_ID is not null  and c.Demand_Note_ID is not null  " +
                                 "group by e.Name ,  cast(a.Settled_On as date) order by Online_Settle_Date , e.Name ,Total_Amount ");
                    //mySql.Append("select e.Name , sum(c.Fee_Amt) as Total_Amount, cast(Settled_On as date) as Online_Settle_Date " +
                    //              "from Online_Transaction  a, Demand_Note b,Course_Registration_Application c, Institute  d, Location e " +
                    //              "where a.Demand_Note_ID =b.ID and b.ServiceID ='" + ServiceCodeRegn + "' and a.Demand_Note_ID =c.Demand_Note_ID and d.ID =c.Institute_ID  and d.State_ID =e.ID  " +
                    //              "and c.Course_Category_Id ='" + CourseCatId + "' and c.Candidate_ID is not null  and c.Demand_Note_ID is not null  " +
            ...
            else if (CourseCode == "C")
            {
                if (ddlAppType.SelectedItem.Text == "Registration")
                {
                    if (ddlDateRange.SelectedItem.Text == "From Start Date")
                    {
                        mySql.Append("select e.Name as 'State_Name', sum(c.Fee_Amt)  as 'Total_Amount', cast(Settled_On as date) as 'Online_Settle_Date'  " +
                ...
                else if (ddlAppType.SelectedItem.Text == "Examination")
                {
                    if (ddlDateRange.SelectedItem.Text == "From Start Date")
                    {
                        mySql.Append("select e.Name as 'State_Name', sum(ce.Fee_Amount)  as 'Total_Amount',  cast(Settled_On as date) as 'Online_Settle_Date'  " +
                ...
                else if (ddlAppType.SelectedItem.Text == "VAF ACF")
                {
                    if (ddlDateRange.SelectedItem.Text == "From Start Date")
                    {
                        mySql.Append("with base as (select dn.ID, dn.Amount, t.Settled_On from Demand_Note dn join Online_Transaction t on dn.ID=t.Demand_Note_ID )                                                                                    " +
                                "  , base2 as (                                                                                                                                                                                                       " +
                                "      SELECT distinct  loc.Name as state_name, cvcf.Institute_ID as institute, b.[ID] as demandnote, b.Settled_On Settle_Date, cvcf.[TotalFee_Amount]   as fee , cvcf.[Course_ID] as CourseID                        " +
                                "      FROM                                                                                                                                                                                                           " +
                                "      Course_Exam_VAF_ACF  cvcf                                                                                                                                                             " +
                                "      inner join  [Institute] ins                                                                                                                                                             " +
                                "      on cvcf.Institute_ID = ins.ID                                                                                                                                                                                  " +
            ...
            else if (CourseCatId == 3)
            {
                if (ddlAppType.SelectedItem.Text == "Registration")
                {
                    if (ddlDateRange.SelectedItem.Text == "From Start Date")
                    {
                        mySql.Append("select e.Name as 'State_Name', sum(c.Fee_Amt)  as 'Total_Amount', cast(Settled_On as date) as 'Online_Settle_Date'  " +
                                        "from Online_Transaction a, Demand_Note b,Course_Registration_Application c, Location e	 " +
                                        "where a.Demand_Note_ID =b.ID and b.ServiceID =  @ServiceCodeRegn and a.Demand_Note_ID =c.Demand_Note_ID  and C.Cor_State_ID =e.ID and " +
                                        "a.Settled_On >= CAST(@fromDate as DATE) group by e.Name ,cast(Settled_On as date) order by Online_Settle_Date desc");
                        //mySql.Append("select e.Name as 'State_Name', sum(c.Fee_Amt)  as 'Total_Amount', cast(Settled_On as date) as 'Online_Settle_Date'  " +
                        //                "from Online_Transaction a, Demand_Note b,Course_Registration_Application c, Location e	 " +
                        //                "where a.Demand_Note_ID =b.ID and b.ServiceID = '" + ServiceCodeRegn + "' and a.Demand_Note_ID =c.Demand_Note_ID  and C.Cor_State_ID =e.ID and " +
                        //                "a.Settled_On >= CAST('" + FromDate.ToString("dd-MMM-yyyy") + "' as DATE) group by e.Name ,cast(Settled_On as date) order by Online_Settle_Date desc");
                    }
                    else
                    {
                        mySql.Append("select e.Name as 'State_Name', sum(c.Fee_Amt)  as 'Total_Amount',  cast(Settled_On as date) as 'Online_Settle_Date'  " +
