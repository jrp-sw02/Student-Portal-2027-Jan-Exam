...
public partial class HO_NielitCentreStudentEdit_HO : BasePage
{
    int stateid = 0;
        ...
        else
        {
            ...
            try
            {
                ...
                using (EConnectContext context = new EConnectContext())
                {
                    ...
                    // UserRefNumber = Convert.ToInt32(loginUser.UserRefNumber);  testing
                    //  UserRefNumber = 5007;
                    UserRefNumber = 5001;
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    {
                        var student = (from p in context1.NielitCentreStudent
                                       where p.Number.ToString() == txtrefno.Text
 ...
 /*   protected void FillddlcentreName()
    ...
    protected void BindEditNewModeData()
    {
        try
        {
            ...
            using (EConnectContext context = new EConnectContext())
            {
                ...
                var PerState = (from s in context.Locations
                                orderby (s.Name)
                                where s.LocationTypeID == 2
                                && s.ParentLocationID == 1
                                select new { ValueField = s.ID, TextField = s.Name }).ToList();
                EConnect.Utils.Common.ControlUtility.BindListObject(ddlCorState , PerState, lst);
    ...
    protected void UpdateData()
    {
		...
		NIELITStudentCentreHistory();
                ...
                if (Convert.ToInt32(ddlWheatherPojectStu.SelectedValue) == 1)
                {
                ...
                objRegistration.projectId = Convert.ToInt32(ddlProcname.SelectedValue);
                ...
                if (UidTypeDdl.SelectedValue.ToString() == "1")
                    objRegistration.UIDNumber = EConnect.Utils.Security.Encryption.Encrypt(UidNumberTxt.Text.Trim());
                else
                ...
                Response.Redirect("NielitCentreStudentEdit_HO.aspx?msg=" + strMessage, true);
            }
        }
    ...
    //}
    //START -- CODE Added on 25 Nov 2022 by DEEP NARAYAN 
    ...
    //END -- CODE Added on 25 Nov 2022 by DEEP NARAYAN
    ...
    protected void bindProject()
    {
        try
        {
            ...
            using (NIELITMISContext context = new NIELITMISContext())
            {
                ...
                //START -- CODE Added on 25 Nov 2022 by DEEP NARAYAN  NIELITMIS 
                Int32 courseCatId = 0;
                string checkDbExistsData = "NA";
                if (cID != 0)
                {
                    //string Query1 = "select courseID from NIELITMIS.dbo.NielitCourseDuration where id=" + cID;

                    //November_2024
                    string Query1 = "select courseID from NIELITMIS.dbo.NielitCourseDuration where id= @cID ";
                    paramList.Add(new SqlParameter("@cID", cID));
    ...
    protected void BindCentre()
    {
        ...
        try
        {
            ...
            {
                var Centre = (from i in context.NielitCentres
                              orderby i.Name
                              select new { ValueField = i.ID, TextField = i.Name }).ToList();
                EConnect.Utils.Common.ControlUtility.BindListObject(ddlCenter, Centre.Distinct(), lst);
    ...
    protected void BindAccCentre()
    {
        try
        {
            ...
            using (var context = new NIELITMISContext())
            {
                var Centre = (from i in context.AffInstitutes 
                              orderby i.Name
                              select new { ValueField = i.instituteID, TextField = i.Name + " - "+i.Accr_No.ToString ()}).ToList();
                EConnect.Utils.Common.ControlUtility.BindListObject(ddlCenter, Centre.Distinct(), lst);
    ...
    protected void BindNonAccCentre()
    {
        try
        {
            ListItem lst = new ListItem("--Select One--", "0");
            using (var context = new NIELITMISContext())
            {
                var Centre = (from i in context.NonAffInstitutes
                              orderby i.Name
                              select new { ValueField = i.ID, TextField = i.Name + " - " + i.CityName  }).ToList();
                EConnect.Utils.Common.ControlUtility.BindListObject(ddlCenter, Centre.Distinct(), lst);
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            if (IsValidForm())
            {
                ...
                if (ddlWheatherPojectStu.SelectedValue == "1" && utility.checkprojectduplicate(name, fname,gender ,dob, courseId, projectId, number, isHO) == 1) //duplicate
                {
    ...
    protected void chkSame_CheckedChanged(object sender, EventArgs e)
    {
        try
        {
            if (chkSame.Checked)
            {
                ...
                TxtCorCity.Text = TxtCorCity.Text;
    ...
    protected void BindBatch(Int64? ID)
    {
        try
        {
            //Added 6 Sep2021
    ...
    protected void btnShow_Click(object sender, System.EventArgs e)
    {
        try
        {
			...
			//Added for non formal courses only
			 NIELITMISContext context11 = new NIELITMISContext();
            string Number1 = txtrefno.Text;
            var application1 = (from a in context11.NielitCentreStudent
                                           
                                           // join i in context.NielitCentreBatch
                                           // on a.centreID equals i.centreID
                                           where a.Number == Number1
                         ...
                         //                  && a.whether_Certificate_Issued == false
                                           ...
                                           select new
                                           {
                                               whetherAffiliated=a.whetherAffiliated ,
                                               CourseId = a.CourseID,
                                               BatchID = a.batch_ID,
            ...
            {
                ShowAlert("Incorrect Online Ref No., please check");
                tralready.Visible = false;
            ...
            if (application1 != null )
            {
                var courseCategory = (from y in context11.NielitCourseDurations
                                     join x in context11.NielitCentreCourses on y.courseID equals x.ID
                                     join i in context11.NielitCentreCourseCategorys
                                     on x.CourseCategoryID equals i.ID
                                     where y.ID == application1.CourseId
                                   //  && i.ID == 102
                                     select new {id=i.ID}).FirstOrDefault();
                
                if (courseCategory != null && courseCategory.id==101)
                {
                    ShowAlert("Student may be of formal course, use Edit for formal courses");
                    return;
                }
            ...
            if (application1.whetherAffiliated  == "O")
            {
                BindCentre();
            }
            if (application1.whetherAffiliated == "Y")
            {
                BindAccCentre();
            }
            if (application1.whetherAffiliated == "N")
            {
                BindNonAccCentre();
            }
            BindBatch(application1.BatchID);

            //NIELITMISContext context1=new NIELITMISContext ();
            var batch = (from a in context11.NielitCentreBatchs
                         where a.ID == application1.BatchID 
                         select new
                         {
                             id = a.ID,
                             centreid = a.centreID,
                             subcentreid = a.subCentreID,
                             ownaff = a.whetherAffiliated
                         }).FirstOrDefault();

            if (batch.ownaff.ToString() == "O")
            {
                ddlCenter.SelectedValue = batch.centreid.ToString();
                ddlCenter.Enabled = false;
                RdoAffInstOrNonAffInst.SelectedValue = "2";
                RdoAffInstOrNonAffInst.Enabled = false;
            }
            if (batch.ownaff.ToString() == "Y")
            {
                ddlCenter.SelectedValue = batch.subcentreid.ToString();
                ddlCenter.Enabled = false;
                RdoAffInstOrNonAffInst.SelectedValue = "1";
                RdoAffInstOrNonAffInst.Enabled = false;
            }
            if (batch.ownaff.ToString() == "N")
            {
                ddlCenter.SelectedValue = batch.subcentreid.ToString();
                ddlCenter.Enabled = false;
                RdoAffInstOrNonAffInst.SelectedValue = "0";
            ...
            ddlBatch.SelectedValue = batch.id.ToString();
                


                    var application = (from a in context11.NielitCentreStudent
                            // join i in context.NielitCentreBatch
                            // on a.centreID equals i.centreID
                            where a.Number == Number1
                            //&& a.whetherPlaced == false
                        ...
                        if (application != null)
                        {
                            ...
                            NielitCourseDuration cd = context11.NielitCourseDurations.Find(application1.CourseId);
                            if(cd!=null)
                            {

                           var course = from y in context11.NielitCourseDurations
                                     join x in context11.NielitCentreCourses on y.courseID equals x.ID
                                     where y.ID == application1.CourseId
                          select new { ValueField = y.ID, TextField = x.Name +"("+ y.courseDurationHrs.ToString () +"Hrs "+y.courseDurationDays.ToString () +" Days)" };
                              ...
                              {
                                  EConnectContext context1=new EConnectContext ();
                                var  course=from y in context1.Courses 
                                         where y.ID ==application1.CourseId 
                                         select new { ValueField = y.ID, TextField = y.Name +"("+ y.Code +")" };
                                  EConnect.Utils.Common.ControlUtility.BindListObject(ddlCourse ,course, lst);
                              }
                            ddlCourse .SelectedValue=application1 .CourseId .ToString ();
                            ddlCourse.Enabled =false;
                                   bindProject ();
