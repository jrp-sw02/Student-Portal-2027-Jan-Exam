...
public partial class HO_Rpt_ProjectBasedStudentsReport : BasePage
{
    ...
    protected void FillCourseCat()
    {
        ...
        try
        {
            ...
            {
                connection.Open();
                using (var command = new SqlCommand("[sp_GetCourseCategory]", connection))
                {
                    command.CommandType = CommandType.StoredProcedure;
                    ...
                    using (SqlDataAdapter da = new SqlDataAdapter(command))
                    {
                        ...
                        EConnect.Utils.Common.ControlUtility.BindListObject(ddlcourse_cat, ds.Tables[0], lst1);
                    }
                }
            }
        }
    ...
    protected void FillCourse()
    {
        ...
        Int64 coursecatid = Convert.ToInt64(ddlcourse_cat.SelectedValue);
        ...
        try
        {
            using (var connection = new SqlConnection(ConfigurationManager.ConnectionStrings["NIELITMISContext"].ConnectionString))
            {
                ...
                using (var command = new SqlCommand("[sp_GetCourse]", connection))
                {
                    command.CommandType = CommandType.StoredProcedure;
                    command.Parameters.Clear();
                    command.Parameters.AddWithValue("@pCourseCategory", coursecatid);
        ...
        try
        {
            ...
            using (var connection = new SqlConnection(ConfigurationManager.ConnectionStrings["EconnectContext"].ConnectionString))
            {
                ...
                using (var command = new SqlCommand("getdataforProjects", connection))
                {
                    ...
                    command.Parameters.AddWithValue("@project_id", Convert.ToInt64(ddlcourse.SelectedValue));
        ...
        try
        {
            ...
            using (var connection = new SqlConnection(ConfigurationManager.ConnectionStrings["EconnectContext"].ConnectionString))
            {
                ...
                using (var command = new SqlCommand("getdataforProjects", connection))
                {
                    ...
                    command.Parameters.AddWithValue("@batch_start", Convert.ToDateTime(txtdate1.Text));
                    command.Parameters.AddWithValue("@batch_end", Convert.ToDateTime(txtdate2.Text));
                    command.Parameters.AddWithValue("@project_id", ddlcourse.SelectedValue);
