...
public partial class Admin_DownloadApplications : BasePage
{
    ...
    protected void ShowEditMode()
    {
        try
        {
            ...
            using (EConnectContext context = new EConnectContext())
            {
                ...
                BreadCrumb1.AddNewBreadCrumbItem(new BreadCrumbItem(applications.Number + "-" + "(" + GetInitCap(applications.Name) + ")", "Admin/DownloadApplications.aspx?" + Request.QueryString.ToString(), ""));
    ...
    protected void ToggleViewMode_Changed(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                Response.Redirect("DownloadApplications.aspx?cname=" + Request.QueryString["CourseID"] + "&exam=" + Request.QueryString["ExamID"] + "&examcycle=" + Request.QueryString["examcycle"] + "&examyear=" + Request.QueryString["examyear"] + "&coursecategory=" + Request.QueryString["coursecategory"], true);
            }
            else
                Response.Redirect("DownloadApplications.aspx", true);
            ...
            if (ddlExamName.SelectedValue != "0")
            {
                using (EConnectContext context = new EConnectContext())
                {
                    ...
                    else
                    {
                        ...
                        var appls = context.CertificateExamApplications.Where(a => a.ExamID == examID && (a.ApplicationStatusID == applDirectStatusID || a.ApplicationStatusID == applInstituteStatusID || a.ApplicationStatusID == applicationStatus) && a.PaymentStatusID != paymentPendingStatusID && a.FinalSubmitted == true && a.DemandNoteID != null
    ...
    protected void btnDownload_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (currentCourse.enmCourseType == enmCourseType.CertificationExam)
            {
                ...
                {
                    string courseCode = currentCourse.Code.ToString();
                    filename = courseCode + "_" + ddlExamName.SelectedItem.Text.Replace(" ", "").Replace(",", "_") + "_" + DateTime.Now.ToString("ddMMyyyyHHmm") + ".mdb";
                }
            }
            ...
            System.IO.File.Copy(Server.MapPath("~/Download/Database_format_BCC.mdb"), mdbFilePath);
            ...
            if (context.Exams.Find(examId).IsBatchProcessable)
            {
                ...
                sql = " SELECT DISTINCT C.ID AS APPL_ID ,C.DATE AS APPL_DATE ,C.NUMBER AS APPL_NUMBER ,'NA' AS FOLDER_NO,BAT.BATCH_NO AS BATCH_NO ,C.ID AS SR_NO," +
                        ...
                        " CASE WHEN C.APPLICANT_TYPE_ID = 1 THEN 'OTH' ELSE 'AIC' END AS INS_STATUS , " +
                        " CASE WHEN C.APPLICANT_TYPE_ID = 1 THEN 'NA' ELSE (SELECT UPPER(NAME) FROM LOCATION  WHERE ID = INS.State_ID )  END AS INS_STATE , " +
                        " CASE WHEN C.APPLICANT_TYPE_ID = 1 THEN 'NA' ELSE (SELECT UPPER(NAME) FROM LOCATION  WHERE ID = INS.District_ID ) END AS INS_DISTRICT , " +
                        " CENTER1.CODE AS EXAM_CENTER1 ,CENTER2.CODE AS EXAM_CENTER2 ,OCC.CODE AS OCCUPATION,CATG.CODE AS CATEGORY," +
                ...
                if (regCentreId != 0)
                    sql = sql + "AND C.REGIONAL_CENTER_ID = " + regCentreId + "";

                sql = sql + " AND C.APPLICATION_STATUS_ID =" + applicationStatus + " AND C.EXAM_CENTER1_ID = CENTER1.ID AND C.EXAM_CENTER2_ID = CENTER2.ID " +
            ...
            if (dtTbl.Rows.Count > 0)
            {
                for (int i = 0; i < dtTbl.Rows.Count; i++)
                {
                    sqlStr = " insert into [MS Access;Database=" + mdbFilePath + "].[Exam_Database] (Application_ID, Application_Date, Application_Number, " +
                                    ...
                                    " CITY, STATE, District, PINCODE, PH_NO_C, EMAIL_C, CCC_NO, INST_NAME, INST_ADD, INST_STATUS,INST_STATE,INST_DISTRICT, TH_CENT_CH," +
                    ...
                    sqlStr += "      ) " +
                                    ...
                                    + dtTbl.Rows[i]["institute_addr"].ToString().Replace("'", "''") + "', '" + dtTbl.Rows[i]["ins_status"] + "', '" + dtTbl.Rows[i]["INS_STATE"].ToString().Trim() + "', '" + dtTbl.Rows[i]["INS_DISTRICT"].ToString().Trim() + "', '" + dtTbl.Rows[i]["exam_center1"].ToString().Trim() + "','" + dtTbl.Rows[i]["exam_center2"].ToString().Trim()
    ...
    protected void btnPendingCount_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            Response.Redirect("DownloadApplicationsPendingCount.aspx?courseID=" + courseId + "&examID=" + examID + "&regionalCentreID=" + regionalCentreID);
        }
