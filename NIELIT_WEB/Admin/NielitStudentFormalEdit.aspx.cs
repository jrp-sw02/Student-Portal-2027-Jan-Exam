...
public partial class Admin_NielitStudentFormalEdit : BasePage
{
    int stateid = 0;
    //added by amit start
            ...
            if (!Page.IsPostBack)
            {
                ...
                using (EConnectContext context = new EConnectContext())
                {
                    ...
                    else
                    {
                        ...
                        {
                            BreadCrumb1.AddNewBreadCrumbItem(new BreadCrumbItem("Nielit Student Formal Edit", "Admin/NielitStudentFormalEdit.aspx?CourseId=" + Request.QueryString["CourseId"].ToString(), ""));
                        }
                        else
                        {
                            BreadCrumb1.AddNewBreadCrumbItem(new BreadCrumbItem("Nielit Student Formal Edit", "Admin/NielitStudentFormalEdit.aspx", ""));
                        }
    ...
    {
        Response.Redirect("NielitStudentFormalEdit.aspx", true);
    ...
    protected void UpdateData()
    {
        try
        {
            ...
            using (NIELITMISContext context = new NIELITMISContext())
            {
                ...
                Response.Redirect("NielitStudentFormalEdit.aspx?msg=" + strMessage, true);
            }
        }
    ...
    public void NIELITStudentCentreHistory()
    {
        try
        {
            ...
            //added by amit testing
            string remarks = "Edit tests by amit";
            // added by amit testing
            string constr = ConfigurationManager.ConnectionStrings["NIELITMISContext"].ConnectionString;
        ...
        try
        {
            ...
            using (TransactionScope tr = new TransactionScope())
            {
                using (var command = new SqlCommand("CheckProjectMapping", con))
                {
                    ...
                    if (ddlProcname != null && ddlProcname.SelectedItem != null)
                    {
    ...
    protected DataTable CheckDocstatus()
    {
        ...
        try
        {
            ...
            INNER JOIN DocumentsMas dm ON ud.DocumentTypeID = dm.id
            WHERE ncs.id = @applID";
    ...
    //    // bindCastCategory(); commented by amit
    ...
    //    if (!String.IsNullOrEmpty(hfapplcode.Value))
    //    {
    //        Int64 applID = Convert.ToInt64(hfapplcode.Value);

    //        DataTable uploadedDocs = new DataTable();
    //        string constr = ConfigurationManager.ConnectionStrings["NIELITMISContext"].ConnectionString;
    //        using (SqlConnection conn = new SqlConnection(constr))
    //        {
    //            string sql = @"SELECT dm.id AS CandID
    //                       FROM NielitCentreStudent ncs
    //                       INNER JOIN UploadedDocs ud ON ud.Student_Id = ncs.ID
    //                       INNER JOIN DocumentsMas dm ON ud.DocumentTypeID = dm.id
    //                       WHERE ncs.id = @applID";
    ...
    //                da.Fill(uploadedDocs);
    //            }
    //        }

    //        // --- If uploads exist, show uploaded grid ---
    //        if (uploadedDocs.Rows.Count > 0)
    //        {
    //            GridView_uploaded.DataSource = dt;
    //            GridView_uploaded.DataBind();
    //            GridView_uploaded.Visible = true;
    ...
    //    else
    //    {
    //        // No applID yet – show upload grid by default
    //        GridView1.DataSource = dt;
    //        GridView1.DataBind();
    //        GridView1.Visible = true;
    //        GridView_uploaded.Visible = false;
                ...
                using (EConnectContext context1 = new EConnectContext())
                {
                    ...
                    using (NIELITMISContext context = new NIELITMISContext())
                    {
                        ...
                        var application = (from a in context.NielitCentreStudent
                                           ...
                                           where a.Number == Number && n.CourseCategoryID==101// Use 101 for server
                        ...
                        // added by amit start
                        hfapplcode.Value = Convert.ToString(application.applId);
                        // added by amit end
                        ...
                        if (application != null)
                        {
                            ...
                            if (UserRefNumber == application.CenterId)
                            {
                                ...
                                if (application.whetherProject.ToString() == "1")
                                {
                                    ...
                                    ddlProcname_SelectedIndexChanged(ddlProcname, EventArgs.Empty);
                                    // added by amit end
                                }
                            ...
                            else if (UserRefNumber != application.CenterId)
                            {
                                ...
                                strMessage = "Student does not belong to this center,please check!!.";
                        ...
                        if (application == null)
                        {
                            ...
                            lblerror.Visible = true;
                            btnNext.Visible = false;
                            lblerror.Text = "Either Reference Number does not exist for this centre or Reference Number does not exist for Formal Course!!.";
                            strMessage = "Either Reference Number does not exist for this centre or Reference Number does not exist for Formal Course!!.";
                        }
                    }
                    ...
                    using (NIELITMISContext context = new NIELITMISContext())
                    {
                        ...
                        var updateLateralId = (from a in context.NielitCentreStudent
                                        ...
                                        select new
                                        {
                                            ...
                                            CourseId = a.CourseID,
                                            SemNo = a.semesterid,
                                            UniversityRegistrationNo = a.University_RegistrationNo,
                        ...
                        if (updateLateralId != null)
                        {
                            if (updateLateralId.LateralEntry == 1)
                            {
                                ...
                                if (updateIdlate != null)
                                {
                                    ...
                                    objRegistration.University_RegistrationNo = updateIdlate.UniversityRegistrationNo;
                        ...
                        else  if (updateLateralId == null)
                        {
                                ...
                                if (updateId != null)
                                {
                                    ...
                                    objRegistration.University_RegistrationNo = updateId.UniversityRegistrationNo;
    ...
    protected void btnNext_Click(object sender, System.EventArgs e)
    {
        try
        {
            ...
            using (NIELITMISContext context = new NIELITMISContext())
            {
                ...
                DataTable dt1 = checkProjectMapping();
                GridView1.Visible = true;
