...
public partial class Admin_NielitCentreStudentMapping : BasePage
{
    ...
    protected void UpdateData()
    {
        try
        {
            ...
            using (NIELITMISContext context = new NIELITMISContext())
            {
                ...
                if(objRegistration.RegisteredCourseRegistrationNo == null)
                {
                    if (rdbCourseType.SelectedValue.Trim() == "R") // added on 29-01-2026
                    {
                    ...
                    else if (rdbCourseType.SelectedValue.Trim() == "A") // added on 29-01-2026
                    {
                        objRegistration.AlreadyRegistered = true;
                        objRegistration.RegisteredCourseID = Convert.ToInt32(ddlCourse.SelectedValue);
                    }
                ...
                {
                    strMessage = "Registration Number is already present. No updation allowed";
                }
    ...
    protected bool IsValidForm()
    {
        try
        {
            ...
            if (txtRegNo.Text == "")
            {
                lblerror.Text = "Please Enter Registration/Application Number.";
                txtRegNo.Focus();
                return false;
            }

            if ( rdbCourseType.SelectedValue=="R" && !Regex.IsMatch(txtRegNo.Text, @"^\d+$"))
            {
                lblerror.Text = "Registration number must contain only digits (0-9).";
                txtRegNo.Focus();
                return false;
            }
