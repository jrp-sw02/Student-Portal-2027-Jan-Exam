...
using EConnect;
public partial class Admin_NielitStudentFormal : BasePage
{
    int stateid = 0;
            ...
            if (!Page.IsPostBack)
            {
                ...
                using (EConnectContext context = new EConnectContext())
                {
                    ...
                    else
                    {
                        ...
                        {
                            BreadCrumb1.AddNewBreadCrumbItem(new BreadCrumbItem("Nielit Student Formal", "Admin/NielitStudentFormal.aspx?CourseId=" + Request.QueryString["CourseId"].ToString() + "&CategoryID=" + Request.QueryString["CategoryID"].ToString(), ""));
                        }
                        else
                        {
                            BreadCrumb1.AddNewBreadCrumbItem(new BreadCrumbItem("Nielit Student Formal", "Admin/NielitStudentFormal.aspx", ""));
                        }
        ...
        try
        {
            using (NIELITMISContext context = new NIELITMISContext())
            {
                ...
                var state = from s in context.NielitCentreStudent
                            join b in context.NielitCourseDurations
                                   on s.CourseID equals b.ID
                            join n in context.NielitCentreCourses on b.courseID equals n.ID
                            join d in context.NielitCentreBatchs on s.batch_ID equals d.ID

                            where s.InstituteID == entityID && s.AlreadyRegistered == false && n.CourseCategoryID == 101 //change for server 101
                            orderby (s.Name)
    ...
    protected void BindGridView()
    {
        ...
        try
        {
            ...
            using (NIELITMISContext context = new NIELITMISContext())
            {
                ...
                NielitCentreId = Convert.ToInt32(Session["EntityID"]);
                var CentreBatchs = from s in context.NielitCentreStudent
                                   join b in context.NielitCourseDurations
                                   on s.CourseID equals b.ID
                                   join n in context.NielitCentreCourses on b.courseID equals n.ID
                                   join d in context.NielitCentreBatchs on s.batch_ID equals d.ID
                                   where s.InstituteID == NielitCentreId && s.AlreadyRegistered == false && n.CourseCategoryID == 101 //change for server 101
                                   select new
                                   {
                                       ID = s.ID,
                                       Name = s.Name,
                ...
                con.Open();
                SqlParameter param;
                using (SqlCommand Cmm = new SqlCommand("BindGridNIELITStudentFormal", con))
                {
                    Cmm.CommandType = CommandType.StoredProcedure;
                    param = new SqlParameter("@enterby", NielitCentreId);
                ...
                {
                    string expression = "[Number] like '%" + searchString + "%'";
                ...
                {
                    string expression = "[ID]=" + Convert.ToInt32(ddlName.SelectedValue);
                    if (DT.Select(expression).Count() > 0)
                        DT = DT.Select(expression).CopyToDataTable();
                    else
                    {
                ...
                if (stateID != 0)
                {
                    CentreBatchs = CentreBatchs.Where(s => s.ID == stateID);
                }
    ...
    protected void ToggleViewMode_Changed(object sender, EventArgs e)
    {
        if (btnMode.ViewMode == ToggleView.Mode.New)
        {
            ...
            lblHeading.Text = "Nielit Student Formal";

            //Updating Breadcrumb
            //BreadCrumb1.AddNewBreadCrumbItem(new BreadCrumbItem("New Exam Centre", "#", ""));
            BreadCrumb1.AddNewBreadCrumbItem(new BreadCrumbItem("New Nielit Student Formal", "", ""));
        }
        else
        {
            if (!String.IsNullOrEmpty(Request.QueryString["CourseId"]))
            {
                Response.Redirect(EConnect.Utils.Security.QuertStringModule.Encrypt("NielitStudentFormal.aspx?ID=" + Request.QueryString["CourseID"].ToString()), true);
            }
            else
            {
                Response.Redirect("NielitStudentFormal.aspx", true);
            }
        }
    }
    ...
    public static String[] GetSearchText(String prefixText, Int32 count)
    {
        ...
        try
        {
            ...
            var Examcentre = from s in context.NielitCentreStudent
                             ...
                             join n in context.NielitCentreCourses on b.courseID equals n.ID
                             join d in context.NielitCentreBatchs on s.batch_ID equals d.ID
                             where s.InstituteID == NielitCentreIdSearch && n.CourseCategoryID == 101 //change for server 101
        ...
        try
        {
            if (!String.IsNullOrEmpty(Request.QueryString["key"]))
            {
                using (NIELITMISContext context = new NIELITMISContext())
                {
                    ...
                    application.semesterid = Convert.ToInt32(ddlsemester.SelectedItem.Text);
        ...
        try
        {
            ...
            using (TransactionScope scope = new TransactionScope())
            {
                using (NIELITMISContext context = new NIELITMISContext())
                {
                    ...
                    //added by amit start
                    DataTable dt2 = checkProjectMapping();
                                    ...
                                    if (row.RowType == DataControlRowType.DataRow)
                                    {
                                        ...
                                        if (fileUpload != null)
                                        {
                                            ...
                                            using (SqlConnection conn = new SqlConnection(ConnectionString))
                                            {
                                                ...
                                                // ShowAlert(fileName0);
                                                using (var command = new SqlCommand("sp_UploadDocs", conn))
                                                {
                    ...
                    Response.Redirect("NielitStudentFormal.aspx?msg=" + strMessage, true);
            ...
            // else
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    using (NIELITMISContext context = new NIELITMISContext())
                    {
                        ...
                        if (rdblateralEntry.SelectedValue == "0")
                        {
                            if (Number != 1)
                            {
                        ...
                        else if (rdblateralEntry.SelectedValue == "1")
                        {
                            if (Number != 3)
                            {
                                var applicationcheck = (from a in context.NielitCentreStudent
                        ...
                        objRegistration.semesterid = Convert.ToInt32(ddlsemester.SelectedItem.Text);
                        int semId = Convert.ToInt32(ddlsemester.SelectedItem.Text);
                        objRegistration.WhetherLateralEntry = Convert.ToInt32(rdblateralEntry.SelectedValue.Trim());
                        if (rdblateralEntry.SelectedValue == "0")
                        {
                            if (semId == 1)
                            {
                                if (txtRegistrationNo.Text.Trim() == "")
                                {
                                    ...
                                    lblerror.Text = "Please Enter University Registration Number.";
                                    return;
                                }
                                else
                                {
                                    objRegistration.University_RegistrationNo = Convert.ToString(txtRegistrationNo.Text); // added by amit
                                }
                            }
                        }
                        else if (rdblateralEntry.SelectedValue == "1")
    ...
    protected bool isDuplicate()
    {
        try
        {
            ...
            using (NIELITMISContext context = new NIELITMISContext())
            {
                ...
                if (courseDurationWithCourseId.ToString().Length > 3)
                {
                ...
                if (Rdoownertype.SelectedValue == "G" && courseID == currentCourseID && courseID != 0)
                {
                    count = (from c in context.NielitCentreStudent
                             ...
                             && c.CourseID == currentCourseID && c.batch_ID == currentbatchID && c.semesterid == Number
    ...
    protected bool IsValidForm()
    {
        try
        {
            ...
            if (ddlsemester.SelectedValue == "0")
            {
                ...
                GenerateNewCaptchaImage();
                txtcode.Text = "";
                lblerror.Text = "Please Select Semester No.";
                ddlsemester.Focus();
                return false;
            }
            ...
            //vishal
            if (!isBlank(txtRegistrationNo))
            {
                tderror.Visible = true;
                lblerror.Visible = true;
                GenerateNewCaptchaImage();
                txtcode.Text = "";
                lblerror.Text = "University Registration Number can not be left blank";
                txtRegistrationNo.Text = "";
                return false;
            }
            /*  if (!isNumber(txtRegistrationNo))
                  {
                  tderror.Visible = true;
                  lblerror.Visible = true;
                  GenerateNewCaptchaImage();
                  txtcode.Text = "";
                  lblerror.Text = "Invalid University Registration Number";
                  txtRegistrationNo.Text = "";
                  return false;
                  }*/
            ...
            if (UIDtr.Visible == true)
            {
                ...
                else
                {
                    GenerateNewCaptchaImage();
                    txtcode.Text = "";
                    throw new Exception("Please select ID Card type..");
                }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            if (IsValidForm())
            {
                ...
                //    UpdateData();
                //}
                //else //if (RadioButtonListCentre.SelectedValue.Trim() == "N")
                //{
                if (!isDuplicate())
                {
                    btnSave.Text = "Submit";
    ...
    protected void RadioButtonListCentre_SelectedIndexChanged(object sender, System.EventArgs e)
    {
        ...
        txtCorPhoneNo.Enabled = true;
        TxtPerAddressLine1.Enabled = true;
        TxtPerAddressLine2.Enabled = true;
        TxtPerAddressLine3.Enabled = true;
        TxtPerCity.Enabled = true;
        ...
        //    ddlsemester.Items.Insert(0, new ListItem("Select", "Select"));
        ...
        //    ResetAll();
        //    ddlsemester.Items.Insert(1, new ListItem("1", ""));
        //}
    }
    ...
    //                using (NIELITMISContext context1 = new NIELITMISContext())
    //                {
    //                    var application = (from a in context1.NielitCentreStudent
    ...
    protected void rdblateralEntry_SelectedIndexChanged(object sender, EventArgs e)
    {
    ...
    protected void ddlsemester_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (rdblateralEntry.SelectedValue == "0")
        {
            if (ddlsemester.SelectedItem.Text == "1")
            {
                trRegNo.Visible = true;
            }
            else
            ...
            if (ddlsemester.SelectedItem.Text == "3")
            {
                trRegNo.Visible = true;
            }
            else
