...
public partial class perfoCriteriasNew : BasePage
{
        ...
        try
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                else
                {
                    ...
                    FillCourseCat();
                    BindGridView();
                    BreadCrumb1.AddNewBreadCrumbItem(new BreadCrumbItem("Performance Criteria New SOP", "Admin/perfoCriteriasNew.aspx", ""));
                }
    ...
    protected void ShowEditMode()
    {
        ...
        try
        {
            EConnect.NIELIT.perfoCriteriaMasNewSOP objPerfoCriteria = context.perfoCriteriaMasNewSOP.Find(Convert.ToInt32(Request.QueryString["Key"]));
            ...
            //ddlstatesCriteria.SelectedValue = objPerfoCriteria.StateCriteria.ToString();
            if (objPerfoCriteria.StateCriteria.Trim() == "O")
            {
                ddlstatesCriteria.SelectedValue = "O";
            }
            else
            {
                ddlstatesCriteria.SelectedValue = "E";
            }
           ...
           txtEffectiveFrom.Text = objPerfoCriteria.effectiveFromDate.ToString("dd-MMM-yyyy");
            ...
            BreadCrumb1.AddNewBreadCrumbItem(new BreadCrumbItem(objPerfoCriteria.ID.ToString () , "Admin/perfoCriteriasNew.aspx?Key=" + Request.QueryString["Key"], ""));
    ...
    { EConnectContext context = new EConnectContext();
        try
        {
            ...
            var query = from s in context.perfoCriteriaMasNewSOP
                       
                        select new
                        {
                            ID = s.ID,
                            courseCat=s.parentCourseCat.Name ,
                            coursecatid=s.courseCatID,
                            cityType=s.parentCityType.description   ,
                            stateCriteria=s.StateCriteria=="O" ? "Others" : "UTs and NE",
                            minCand=s.minCand ,
            ...
            if (courseCategory != 0)
                query = query.Where(s => s.coursecatid == courseCategory);
    ...
    protected void ToggleViewMode_Changed(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("perfoCriteriasNew.aspx", true);
        }
    }
    ...
    protected void SaveRecord(object sender, EventArgs e)
    {
         EConnectContext context = new EConnectContext();
        //create an object 
          perfoCriteriaMasNewSOP objPerfoCriteria=new perfoCriteriaMasNewSOP ();
        try
        {
            ...
            if (String.IsNullOrEmpty(Request.QueryString["Key"]))
            {
                ...
                string statecriteria=ddlstatesCriteria.SelectedValue;
                if (context.perfoCriteriaMasNewSOP.Where(s => s.courseCatID == cId && s.StateCriteria == statecriteria && s.effectiveToDate == null).Count() != 0)
                {
                                
                    throw new Exception("This Criteria is Already Exists");
                }
                else
                {       
                    objPerfoCriteria = new perfoCriteriaMasNewSOP();
                    Int32 cityTypeId = 1;// Convert.ToInt32(ddlCityType.SelectedValue);
                    ...
                    objPerfoCriteria.cityTypeID = cityTypeId;
                    objPerfoCriteria.StateCriteria = ddlstatesCriteria.SelectedValue;
                    ...
                    context.perfoCriteriaMasNewSOP.Add(objPerfoCriteria);
                    context.SaveChanges();
            ...
            else
            {
                ...
                DateTime eDate = DateTime.ParseExact(txtEffectiveFrom.Text, "dd-MMM-yyyy", CultureInfo.InvariantCulture);
                objPerfoCriteria = context.perfoCriteriaMasNewSOP.Find(KeyID);
            ...
            Response.Redirect("perfoCriteriasNew.aspx?msg=" + strMessage);
        }
        catch (Exception ex)
        {
            ShowAlert(ex.Message, true);
