...
public partial class NielitCentreStudent_Verify_Info : BasePage
{
            ...
            if (!Page.IsPostBack)
            {
                ...
                using (EConnectContext context = new EConnectContext())
                {
                    ...
                    using (NIELITMISContext context1 = new NIELITMISContext())
                    {
                        ...
                        {
                            BreadCrumb1.AddNewBreadCrumbItem(new BreadCrumbItem("NielitCentreStudent: Application Status", "Admin/NielitCentreStudent_Verify_Info.aspx?Id=" + Request.QueryString["Id"].ToString(), ""));
                        }
                        else
                        {
                            if (BatchIDs != 0)
                            ...
                            BreadCrumb1.AddNewBreadCrumbItem(new BreadCrumbItem("NielitCentreStudent: Application Status", "NielitCentreStudent_Verify_Info.aspx", ""));
                        }
                    }
                }
            }
    ...
    // added on 13 april 2021 by deep
    public DataTable FillCourseCatNIELITMISCourseNielitCourseCat()
    {
    ...
    public DataTable FillCourseNIELITMISCourseNielitCourse(int CourseCat)
    {
        ...
        using (SqlConnection con = new SqlConnection(constr))
        {
            try
            {
                using (SqlCommand cmd = new SqlCommand("FillCourseNIELITMISCourseNielitCourseVerify", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add(new SqlParameter("@CourseCat", SqlDbType.Int));
                    cmd.Parameters["@CourseCat"].Value = CourseCat;
                    con.Open();
                    using (SqlDataAdapter sda = new SqlDataAdapter(cmd))
                    {
                        sda.Fill(myDt);
    ...
    protected void FillFilterCourseCategory()
    {
        try
        {
            using (NIELITMISContext context = new NIELITMISContext())
            {
                ...
                using (DataTable dt = FillCourseCatNIELITMISCourseNielitCourseCat())
                {
                    if (dt.Rows.Count > 0)
                    {
                        ddlCourseCategoryName.DataSource = dt;
                        ddlCourseCategoryName.DataTextField = "Name";
                        ddlCourseCategoryName.DataValueField = "ID";
                        ddlCourseCategoryName.DataBind();
                        ddlCourseCategoryName.Items.Insert(0, new ListItem("--Select One--", "0"));
                    }
                }
            };
        }
        ...
        try
        {
            using (NIELITMISContext context = new NIELITMISContext())
            {
                if (catID != null || catID != 0)
                {
                    ...
                    using (DataTable dt = FillCourseNIELITMISCourseNielitCourse(catID))
                    {
                        if (dt.Rows.Count > 0)
                        {
            ...
            using (NIELITMISContext context = new NIELITMISContext())
            {
                ...
                else
                {
                    var BatchName = from p in context.NielitCentreBatchs
                                        //join k in context.NielitCourseDurations on p.CourseDurationID equals k.ID
                                        //where p.IsVerified == true && k.isVerified == true && k.courseID == coursenameid
                                    where p.CourseDurationID == coursenameid
                                            && p.centreID == NielitCentreIdFilter
                                    orderby (p.Name)
                                    select new { ValueField = p.ID, TextField = p.Name };
                    ddlbatchname.Items.Clear();
    ...
    protected void ddlIsverifiedStatus_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            using (NIELITMISContext context = new NIELITMISContext())
            {
                if (IsverifyStatus == 1)
                {
                    //Added for project based
                    var candProject = (from p in context.NielitCentreStudent
                                       join d in context.NielitCentreBatchs on p.batch_ID equals d.ID
                                       where d.ID == bathcid && p.batch_ID == bathcid && p.ID == studentid
                    ...
                    if (candProject != null)
                    {
                        vProjectID = candProject.projectId.ToString();
                    }
                    ...
                    var CheckAadharAuthRequred = (from p in context.NielitProjCoursess
                                                  ...
                                                  where p.courseID == courseid && k.isAadharAuthenticationReqd == true
                        && p.projID.ToString() == vProjectID
                                                  select p).Count();
                    if (CheckAadharAuthRequred != 0)
                    {
                        ...
                        var CheckAadharAuthStudent = (from p in context.NielitCentreStudent
                                                      join d in context.NielitCentreBatchs on p.batch_ID equals d.ID
                                                      where d.ID == bathcid && p.batch_ID == bathcid && p.ID == studentid && p.AadharVerfied == false
                                                      select p).Count();
        ...
        try
        {
            ...
            using (NIELITMISContext context = new NIELITMISContext())
            {
                var courseStudentData = (from s in context.NielitCentreStudent
                                         ...
                                         where b.centreID == NielitCentreIdFilter && s.batch_ID == BatchID || s.batch_ID == BatchIDh
                ...
                //if (verifiedid == 0 )
                //    {
                //        courseStudentData = courseStudentData.Where(s => s.VerifiedStatusID == 99);
                //    }
    ...
    protected void ToggleViewMode_Changed(object sender, EventArgs e)
    {
        if (btnMode.ViewMode == ToggleView.Mode.New)
        {
            ...
            lblHeading.Text = "New NielitCentreStudent";
            //Updating Breadcrumb
            BreadCrumb1.AddNewBreadCrumbItem(new BreadCrumbItem("NielitCentreStudent: Application Status", "", ""));
        }
        else
        {
            if (!String.IsNullOrEmpty(Request.QueryString["Id"]))
    ...
    protected void btnback_Click(object sender, EventArgs e)
    {
        ...
        if (this.ViewState["BatchID"] == null)
        {
            ...
            if (BatchIDh != 0)
            {
                ...
                Response.Redirect(EConnect.Utils.Security.QuertStringModule.Encrypt("NielitCentreStudent_Verify_Info.aspx?BatchID=" + BatchIDh), true);
            }
        }
    ...
    protected void ResetFilterPanel(object sender, EventArgs e)
    {
        try
        {
            ...
            BreadCrumb1.AddNewBreadCrumbItem(new BreadCrumbItem("Application Status:", "Admin/NielitCentreStudent_Verify_Info.aspx", ""));
            BreadCrumb1.Render();
        }
        catch (Exception ex)
        {
