...
public partial class EditCandidateDetail : BasePage
{
            ...
            if (loginUserType == UserType.RegionalCenter || loginUserType == UserType.HeadOffice || loginUserType == UserType.Admin || loginUserType == UserType.ExternalAdmin)
            {
                if (!IsPostBack)
                {
                    ...
                    else
                    {
                        ...
                        {
                            BreadCrumb1.RemoveLastBreadCrumbItem();
                            BreadCrumb1.AddNewBreadCrumbItem(new BreadCrumbItem("Edit Candidate(CCC) Data", "Admin/EditCandidateDetail.aspx", ""));
                            BreadCrumb1.Render();
                        }
                    }
                }
                //else
                //{
                //    BreadCrumb1.AddNewBreadCrumbItem(new BreadCrumbItem("Guardian Name", "#", ""));
                //    btnView.Visible = false;
                //    btnReset.Visible = false;
                //    Lblerror.Text = "You can not view records.";
    ...
    protected void btnReset_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            gvMain.DataBind();
            Response.Redirect("EditCandidateDetail.aspx");
        }
        catch (Exception ex)
        {
            ShowAlert(ex.Message, true);
    ...
    protected void ShowEditMode()
    {
        try
        {
            ...
            BreadCrumb1.AddNewBreadCrumbItem(new BreadCrumbItem("Edit Candidate(CCC) Data:Update", "", ""));
            BreadCrumb1.Render();
            context = new EConnectContext();
           ...
           /* if (student.gender.ToString().ToUpper() == "FEMALE")
            ...
            //Added for gender
            ddlGender.SelectedValue = student.gender.ToString().Trim();
    ...
    protected void SaveRecord(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!String.IsNullOrEmpty(Request.QueryString["ApplID"]))
            {
                if (!string.IsNullOrWhiteSpace(TxtRemark.Text) == true)
                {
                    string sql = "insert into CertificateExamApplication_CandidatePersonalDetailUpdationHistory(" +
                                        "[ID],[Date],[Number],[Candidate_ID],[Already_Applied],[Previous_Exam_ID],[Previous_Roll_Number],[Previous_Application_ID]," +
                                        "[Course_Category_ID],[Course_ID],[Applicant_Type_ID],[Exam_ID],[Institute_ID],[Exam_Center1_ID],[Exam_Center2_ID]," +
                                        "[Regional_Center_ID],[Salutaion],[Name],[Father_Name],[Mother_Name],[Gender],[Dob],[Cast_Category_ID],[Occupation_ID]," +
                                        "[Photo_File_Name],[Photo],[Signature_File_Name],[Signature],[Left_Thumb_File_Name],[Left_Thumb],[Educational_Qualification_ID]," +
                                        "[Passing_Year],[Mobile],[Std],[Phone],[Email],[Cor_Address1],[Cor_Address2],[Cor_Address3],[Cor_Country_ID],[Cor_State_ID],[Cor_District_ID]," +
                                        "[Cor_City_Name],[Cor_Pin_Code],[Final_Submitted],[Final_Submission_Date],[Demand_Note_ID],[Course_Duration_From],[Course_Duration_To]," +
                                        "[Fee_Type_ID],[Fee_Amt],[Late_Fee_Amt],[Total_Fee_Amt],[Is_Verified_By_Institute],[Verified_On_By_Institute],[Payment_Status_ID]," +
                                        "[Application_Status_ID],[Batch_Item_ID],[Roll_Number],[Exam_Centre_Name],[Exam_Centre_Address],[Date_of_Exam],[Exam_Batch_Number]," +
                                        "[Reporting_Time],[Updated_On],[Updated_By],[Guardian_Name],[Result_Grade_ID],[Result_Updated_On],[Result_Updated_By],[Exempted_Application_ID]," +
                                        "[Is_Exempted],[App_Source],[Is_Synced],[Synced_On],[Exam_Month],[Exam_Year],[Examn_Cycle_ID],[Previous_Exam_Name],[Aadhar_Number],[Aadhar_Verfied]," +
                                        "[Department],[Employee_Code],[Designation],[Posting_City],[Date_of_Joining],[Date_of_Retirement],[Is_Downloaded],[Downloaded_On],[Updated_on_New], [Updated_by_New], [Remark_for_Update] )" +

                                        "(select [ID],[Date],[Number],[Candidate_ID],[Already_Applied],[Previous_Exam_ID],[Previous_Roll_Number],[Previous_Application_ID]," +
                                        "[Course_Category_ID],[Course_ID],[Applicant_Type_ID],[Exam_ID],[Institute_ID],[Exam_Center1_ID],[Exam_Center2_ID]," +
                                        "[Regional_Center_ID],[Salutaion],[Name],[Father_Name],[Mother_Name],[Gender],[Dob],[Cast_Category_ID],[Occupation_ID]," +
                                        "[Photo_File_Name],[Photo],[Signature_File_Name],[Signature],[Left_Thumb_File_Name],[Left_Thumb],[Educational_Qualification_ID]," +
                                        "[Passing_Year],[Mobile],[Std],[Phone],[Email],[Cor_Address1],[Cor_Address2],[Cor_Address3],[Cor_Country_ID],[Cor_State_ID],[Cor_District_ID]," +
                                        ...
                                        "[Reporting_Time],[Updated_On],[Updated_By],[Guardian_Name],[Result_Grade_ID],[Result_Updated_On],[Result_Updated_By],[Exempted_Application_ID]," +
                                        "[Is_Exempted],[App_Source],[Is_Synced],[Synced_On],[Exam_Month],[Exam_Year],[Examn_Cycle_ID],[Previous_Exam_Name],[Aadhar_Number],[Aadhar_Verfied]," +
                                        "[Department],[Employee_Code],[Designation],[Posting_City],[Date_of_Joining],[Date_of_Retirement],[Is_Downloaded],[Downloaded_On],'" + DateTime.Now + "','" + Convert.ToInt32(Session["UserID"]) + "', '" + TxtRemark.Text + "' from Certificate_Exam_Application Where ID=" + Appid + ")";
                    ...
                    student.UpdatedByID = Convert.ToInt32(Session["UserID"]);
                    if (ImgUpload.HasFile)
                    {
                        if (ImgUpload.FileName.Length > (50 * 1024))
                        {
                            student.PhotoFileName = ImgUpload.FileName.Substring(0, 45) + System.IO.Path.GetExtension(ImgUpload.FileName).ToLower();
                        }
                        else
                            student.PhotoFileName = ImgUpload.FileName;
                        student.Photo = ImgUpload.FileBytes;
                    }
                    else
                    {
                        student.Photo = student.Photo;
                        student.PhotoFileName = student.PhotoFileName;
                    }
