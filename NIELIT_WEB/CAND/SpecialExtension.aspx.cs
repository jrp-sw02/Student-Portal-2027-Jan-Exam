...
public partial class SpecialExtension : BasePage
{
        ...
        try
        {
          ...
          //  divPopup.Style.Add("display", "none");
            ...
            using (EConnectContext context = new EConnectContext())
            {
                ...
                {
                    // hdnCandidateID.Value = id.ToString();
            ...
            if (!IsPostBack)
            {
                ...
                //int applicantTypeId = GetApplicantType(id, courseId);
                ...
                {
                    bool isEligible = BindMercyData(candidateId, courseId,"SE");

                    if (!isEligible)
                    {
                        lblerror.Text = "You are not eligible for Special Extension.";
                        Btnsubmit.Visible = false;
...
//"alert('Centres: " + AccCentre.Count + "');", true);
    ...
    protected void Btnsubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand("sp_Insert_MercyApplication", con))
            {
                ...
                {
                    InsertDocument(candidateId, Fumedical, "PROOF OF MEDICAL ISSUES/AFFIDAVIT");
                }
                //if (FuIdCard.HasFile)
                //{
                ...
                Response.Redirect("../CAND/SpecialExtensionPreview.aspx?id=" + candidateId
                        + "&courseId=" + courseId
    ...
    private void SetReUploadMode(bool isReUpload)
    {
        if (isReUpload)
        {
            ...
            lblRegFCourse.Text = "Course / पाठ्यक्रम";
        ...
        {
            lblMainHeading.InnerText = "SPECIAL EXTENSION REGISTRATION FORM";
        ...
        try
        {
            ...
            if (examID != 0)
            {
                using (EConnectContext context = new EConnectContext())
                {
                    ...
                    int FeeTypeID = Convert.ToInt32(enmFeeType.SpecialExtensionRegistrationFee);
                    LblFeeTypeName.Text = Convert.ToString(enmFeeType.SpecialExtensionRegistrationFee);
                    int ExamId = examID;
                    Int32 feeAmount = 0;
