            ...
            using (EConnectContext context = new EConnectContext())
            {
                ...
                if (currentCourse.enmCourseType == enmCourseType.CertificationCourse)
                {
                    ...
                    if (download != null)
                    {
                        ...
                        //else
                        //{
                       //-- Lbename.Text = (!string.IsNullOrEmpty(download.Exam.Name.ToUpper())) ? "CANDIDATE ADMIT CARD <br /> VALID FOR " + download.Exam.Name.ToUpper() + "  EXAMINATION <br /> RESCHEDULED IN AUGUST 2023 " : "";

                        Lbename.Text = (!string.IsNullOrEmpty(download.Exam.Name.ToUpper())) ? "CANDIDATE ADMIT CARD <br /> VALID FOR " + download.Exam.Name.ToUpper() + "  EXAMINATION" : "";
                        //}
                        ...
                        if (download.Exam.DateOfPublishingOfRollNumber.HasValue)
                        {
                           // lbpubdate.Text = download.Exam.DateOfPublishingOfRollNumber.Value.ToString("dd-MMM-yyyy");
                            //lbpubdate.Text = DateTime.Now.ToString("dd-MMM-yyyy");
                        }
                        else
                        {
                           // lbpubdate.Text = "NA";
                        ...
                        var examcentreaddress = (from c in context.ExamVenues
                                                 ...
                                                 where c.ID == download.AllottedExamVenueID && d.ID == download.AllottedExamCentreID && c.CourseID == currentCourseID
                                                 select new
                                                 {
                                                     venuename = c.Name,
                        ...
                        {
                            Lbccode.Text = (!string.IsNullOrEmpty(examcentreaddress.code)) ? examcentreaddress.code.ToUpper() : "";
        ...
        try
        {
            ...
            using (EConnectContext context = new EConnectContext())
            {
                ...
                var examdate = (from r in context.ExamTimeTables
                                ...
                                && r.Module.ModuleTypeID != practical && r.CourseID != 1 && r.ModuleID != 938 && r.ModuleID != 939 && r.ModuleID != 940 && r.ModuleID != 941
                ...
                foreach (ExamSession examSession in examSessions)
                {
                    ...
                    if (appl.CourseID == 1)
                    {
                        //tdSession.Text = "<u><b>" + examSession.Name.ToUpper() + "</b></u>" + WebUtility.HtmlDecode("<br/>") + examSession.StartTime.ToString("HHmm") + "-" + examSession.EndTime.AddHours(-1).ToString("HHmm") + " hrs";
                    }
                    
                    else if (appl.CourseID == 2 && examSession.Name.ToUpper() == "AFTERNOON") 
                    {
