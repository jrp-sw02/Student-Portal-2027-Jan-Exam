            ...
            using (EConnectContext context = new EConnectContext())
            {
                ...
                if (currentCourse.enmCourseType == enmCourseType.CertificationCourse)
                {
                    ...
                    if (download != null)
                    {
                        ...
                        //    download.ExamID == 4903 || download.ExamID == 4905 || download.ExamID == 4907 || download.ExamID == 4909)
                        ...
                        if (download.ExamID == 4246 || download.ExamID == 4248 || download.ExamID == 4250 || download.ExamID == 4252)
                        {
                            ...
                            Lbename.Text = (!string.IsNullOrEmpty(newExam.examname.ToUpper() )) ? "VALID CANDIDATE ADMIT CARD " + newExam.examname.ToUpper() + " EXAMINATION ONLY" : "";
                        }
                        else
                        {
                            Lbename.Text = (!string.IsNullOrEmpty(download.Exam.Name.ToUpper())) ? "VALID CANDIDATE ADMIT CARD " + download.Exam.Name.ToUpper() + " EXAMINATION ONLY" : "";
                        }
        ...
        try
        {
            ...
            var newExam = (  from m in context.Exams
             ...
             if (applmerge == null)
             {
                 ...
                 string moduleName21 = null;
                 string constr1 = ConfigurationManager.ConnectionStrings["EConnectContext"].ConnectionString;
                 SqlConnection con1 = new SqlConnection(constr1);

                 string sql21 = " select (select short_name from module where id =module_id) from Exam_Time_Table where Exam_ID = '" + newExam.examId + "' and Exam_From_Date ='" + examDate + "'  and Exam_Sessiion_ID =' " + examSessionID + "'" +
                                " and module_id in (select Module_ID  from Course_Exam_Application_Detail  where  (Course_Exam_Appl_ID = '" + courseExamApplicationID + "'  ) " +
                                " and (result_grade_id != 7 or result_grade_id is null) and (result_grade_id != 183 or result_grade_id is null)  ) ";

                 using (SqlConnection conn = new SqlConnection(constr1))
                 {
                     using (SqlCommand cmd = new SqlCommand(sql21))
                     {
                         try
                         {
                             cmd.Connection = conn;
                             cmd.Connection.Open();
                             moduleName21 = cmd.ExecuteScalar().ToString();
                           if(  String.IsNullOrEmpty(moduleName21) == null)
                             //if (moduleName21 == null)
                             {
                                 return "";
                             }
                         }
            ...
            string sql = "select (select short_name from module where id =module_id) from Exam_Time_Table where Exam_ID = '" + newExam.examId + "' and Exam_From_Date ='" + examDate + "'  and Exam_Sessiion_ID =' "+ examSessionID+"'" +
                         " and module_id in (select Module_ID  from Course_Exam_Application_Detail  where  (Course_Exam_Appl_ID = '" + courseExamApplicationID + "' or Course_Exam_Appl_ID = '" + applmerge.CourseExamApplicationID + "' ) " +
                         "and (result_grade_id != 7 or result_grade_id is null) and (result_grade_id != 183 or result_grade_id is null)  ) ";
            ...
            using (SqlConnection conn = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand(sql))
                {
                    try
                    {
                        cmd.Connection = conn;
                        cmd.Connection.Open();                       
                        moduleName = cmd.ExecuteScalar().ToString();
                        if (moduleName == null)
                        {
                            return "";
                        }
        ...
        try
        {
            ...
            using (EConnectContext context = new EConnectContext())
            {
                ...
                var examdate = (from r in context.ExamTimeTables
                                where r.ExamID == newExam.examId //2676 
