...
public partial class FrmExamForm_Edit : BasePage
{
   ...
   readonly int[] practicalModules_C = { 444,445,446,447};
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        
        {
            ...
            string dft = Request.QueryString["examStartDate"];
                ...
                using (EConnectContext context = new EConnectContext())
                {
                    ...
                    else
                    {
                        ...
                        var registrationDetail = (from s in context.RegistrationDetails
                                                  where s.CourseID == currentCourseID && s.CandidateID == entityID && s.RegistrationNo == registrationNumber && (s.RegistrationStatus.ID==1||s.RegistrationStatus.ID==2)
                        ...
                        if (registrationDetail != null)
                        {
                            ...
                            Exam exam = (from m in context.Exams where m.ID == ExamId select m).FirstOrDefault();
                            ...
                            if (regStatusID.Contains(registrationDetail.RegistrationStatusID.Value) && registrationDetail.ValidUptoDate >= DateTime.Now.Date)
                            {
                                ...
                                else if (CourseManager.IsAlreadyApplied(context, exam.ID, currentCourseID, registrationNumber, entityID) != max_revision)
                                {
                                    if (!CourseManager.IsNewRegisteredCandidate(currentCourseID, registrationNumber))
                                    {
                                        ...
                                        {
                                            currentevisionNumber = CourseManager.IsAlreadyApplied(context, ExamId, currentCourseID, registrationNumber, entityID);
                                        ...
                                        else
                                       {
                                        ...
                                        if (context.CourseExamApplications.Any(c => (c.ExamID == exam.ID && c.CourseID == currentCourseID && c.CandidateID == entityID && c.RegistrationNumber == registrationNumber)))
                                        {
                                            if (Request.QueryString["Appid"] == null)
                                            {
                                                ...
                                                lblerror.Text = "This is one time edit window to capture for correct center choice. Please select the center choice carefully.<br> for updation please click on update button. ";
    ...
    protected void FillRemianingModules(Int32 courseID, Int64 registrationNumber, Int64 candidateID, Int32 applicantTypeID, Int32? examID, Int32? lastExam_attempted_exceptionalcase, int AfterExempt)
    {
        using (EConnectContext context = new EConnectContext())
        {
            ...
            else 
            {
                if (AfterExempt != 1)
                {
                    if ((courseID == 3 && rbtn_module_selection.SelectedValue == "current") || courseID==2)
                    {
                        revisionNumber = currentevisionNumber;
            ...
            if (modules.Count() >= 0)
            {
                ...
                countPassedCodModule = PassedConditionalTheoryModuleCount(currentCourseID, registrationNumber, revisionNumber, entityID);
                countPassedCodModule_alevel = PassedConditionalTheoryModuleCount_Alevel(currentCourseID, registrationNumber, revisionNumber, entityID);
                if (courseID == 3 && rbtn_module_selection.SelectedValue == "current")
                {
                ...
                if ((courseID == 1 || courseID == 2 ) && (Eligibility_Method_For_Apply_In_Old_Exam_Pattren_O_LVL == 0 || Eligibility_Method_For_Apply_In_Old_Exam_Pattren_A_LVL == 0))
                {
                    lblerror.Visible = true;
                ...
                else if ((courseID == 1 || courseID == 2 ) && (Eligibility_Method_For_Apply_In_Old_Exam_Pattren_O_LVL == 1 || Eligibility_Method_For_Apply_In_Old_Exam_Pattren_A_LVL == 1))
                {
                    lblerror.Visible = true;
                ...
                if (courseID == 3 && Eligibility_Method_For_Apply_In_Old_Exam_Pattren_B_LVL == 0 )
                {
                    lblerror.Visible = true;
                    lblerror.Text = "You are eligible to apply only for new Exam Pattern.";
                    trModuleOption1.Visible = false;
                ...
                con.Close();
                if (countPassedCodModule != 0 && rbtn_module_selection.SelectedValue == "current" && AfterExempt==0 && CheckExemptioDoneOrNot(courseID,registrationNumber,revisionNumber,candidateID)<1)
                {
    ...
    public static ICollection<Module> GetRemainingModules(EConnectContext context, Int32 courseID, Int64 registrationNumber, Int32 currentRevisionNumber, Int64 candidateID, int afterExmpt)
    {
        try
        {
            ...
            //--added code dated 26042023 for A Level (Removing specialized modeule from exemption list for those candidate who is sleigible for exempetion
    ...
    //            }
    //            if (DdlExamCentre2.SelectedValue == DdlExamCentre2.SelectedValue && DdlExamCentre2.SelectedValue != "0")
    //            {
    ...
    //                string constr = ConfigurationManager.ConnectionStrings["EConnectContext"].ConnectionString;
    //                SqlConnection con = new SqlConnection(constr);
    //                string sql = "update course_exam_application set Exam_Center1_ID='" + DdlExamCentre1.SelectedValue + "', Exam_Center2_ID='" + DdlExamCentre2.SelectedValue + "', NSQF_Roll_no ='Y' where Registration_Number='" + registrationNumber + "' and Exam_ID=" + currentExam.ID + "";
    //                con.Open();
    ...
    //        Response.Redirect(EConnect.Utils.Security.QuertStringModule.Encrypt("ExamFormPreview.aspx?AppID=" + applID + "&examStartDate=" + examStartDate.ToString()), true);
    //    }
    //    catch (Exception ex)
    //    {
    //        ShowAlert(ex.Message);
           ...
           // ValidatePracticalModules(0);
            ...
            using (TransactionScope scope = new TransactionScope())
            {
                ...
                using (EConnectContext context = new EConnectContext())
                {
                   ...
                   // Exam currentExam = CourseManager.GetNextExam(context, currentCourseID, registrationDetail.ApplicantTypeID);
                    int ExamId = (from E in context.CourseExamApplications where E.CourseID == currentCourseID && E.CandidateID == entityID && (E.ApplicationStatusID == 10 || E.ApplicationStatusID == 8) && (E.PaymentStatusID == 2 || E.PaymentStatusID == 4) orderby E.ExamID descending select E.ExamID).FirstOrDefault();
                    Exam currentExam = (from m in context.Exams where m.ID == ExamId select m).FirstOrDefault();
            ...
            Response.Redirect(EConnect.Utils.Security.QuertStringModule.Encrypt("ExamFormPreview.aspx?AppID=" + appl.ID.ToString() + "&examStartDate=" + examStartDate.ToString()+"&Chk=Y"), false);
        }
        catch (Exception ex)
        {
            ShowAlert(ex.Message);
    ...
    protected void ValidatePracticalModules( int sts)
    {
        ...
        try
        {
            using (EConnectContext context = new EConnectContext())
            {
                ...
                if (rbtn_module_selection.SelectedValue == "current" || CourseManager.IsAlreadyApplied(context, exam.ID, currentCourseID, registrationNumber, entityID) == currentevisionNumber)
                {
                ...
                if (rbtn_module_selection.SelectedValue == "pre" || CourseManager.IsAlreadyApplied(context, exam.ID, currentCourseID, registrationNumber, entityID) == preevisionNumber)
                {
                ...
                if (remainingTheoryModuleCount > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!(currentCourseID == 4 && firstRevisionNumber < currentevisionNumber && currentevisionNumber == 3) || currentCourseID != 4)
                        {
                            ...
                            if (rbtn_module_selection.SelectedValue == "pre" || (CourseManager.IsAlreadyApplied(context, exam.ID, currentCourseID, registrationNumber, entityID) == preevisionNumber && currentevisionNumber!=5))
                            {
    ...
    protected void ValidateTheoryModules()
    {
        ...
        try
        {
            using (EConnectContext context = new EConnectContext())
            {
                ...
                else if (rbtn_module_selection.SelectedValue == "pre" || CourseManager.IsAlreadyApplied(context, exam.ID, currentCourseID, registrationNumber, entityID) == preevisionNumber)
                {
                    remainingTheoryModuleCount = GetRemainingAllTheoryModuleCount(currentCourseID, registrationNumber, preevisionNumber, entityID);
                }
                else
    ...
    protected void rbtn_module_selection_SelectedIndexChanged(object sender, EventArgs e)
    {
